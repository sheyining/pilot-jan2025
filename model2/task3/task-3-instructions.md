...
The operation `AddTag[net0, net1, c, u]` constrains `net1` to be the network state after adding a tag from content `c` to user `u` in `net0`, in a way that preserves the invariant.
...
The implementation of the operation `AddTag` contains a bug. Therefore, the diagrams you will see show an execution of this buggy version of `AddTag`. Your task is to use the diagrams to diagnose the bug.
