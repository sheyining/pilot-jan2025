...
The operation `AddFriend[net0, net1, u0, u1]` constrains `net1` to be the state after adding the friendship relation between existing users `u0` and `u1`, while maintaining the invariant.
...
The implementation of the operation `AddFriend` contains a bug. Therefore, the diagrams you will see show an execution of this buggy version of `AddFriend`. Your task is to use the diagrams to diagnose the bug.
